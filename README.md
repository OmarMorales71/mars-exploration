...
│   ├── input/
│   │   ├── mission_report.md
│   │   ├── mars_terrain.graphml
│   │   ├── rovers.json
│   │   └── drones.json
│   ├── intermediate/ # All the outputs generated by the crews
│   └── output/ # Final mission strategy
├── commons/
│   ├── llm.py
```
...
* Goals processed by **priority** (high → low)
* **Balancing rule**: avoid assigning the same vehicle to all goals
* Tie-breakers:

  * Rovers: lower energy → shorter distance
  * Drones: higher camera → higher altitude → lower time
...
If you have questions or want to extend the system, start by adding a new crew or tool — the architecture is designed for it.
